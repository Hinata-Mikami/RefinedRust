...
Section trans.
  ...
  Lemma iterator_next_fused_trans_app {Self_rt Item_rt : RT} (A : traits_iterator_Iterator_spec_attrs Self_rt Item_rt) s1 hist s2 x :
    IteratorNextFusedTrans A s1 (hist ++ [x]) s2 ⊣⊢ (∃ s2', IteratorNextFusedTrans A s1 hist s2' ∗ A.(traits_iterator_Iterator_Next) s2' (Some x) s2).
  Proof.
  ...
  Lemma simplify_goal_iterator_next_fused_trans_app {Self_rt Item_rt : RT} (A : traits_iterator_Iterator_spec_attrs Self_rt Item_rt) s1 hist s2 s1' hist' `{!CheckOwnInContext (IteratorNextFusedTrans A s1 hist' s1')} T :
    (∃ x, 
      ⌜hist = hist' ++ [x]⌝ ∗
      IteratorNextFusedTrans A s1 hist' s1' ∗
      A.(traits_iterator_Iterator_Next) s1' (Some x) s2) ∗ T ⊢ simplify_goal (IteratorNextFusedTrans A s1 hist s2) T.
  Proof.
  ...
  Lemma simplify_goal_iterator_next_fused_trans_nil {Self_rt Item_rt : RT} (A : traits_iterator_Iterator_spec_attrs Self_rt Item_rt) s1 hist T :
    (⌜hist = []⌝) ∗ T ⊢ simplify_goal (IteratorNextFusedTrans A s1 hist s1) T.
  Proof.
    rewrite /simplify_goal.
    iIntros "(-> & $)".
    eauto.
