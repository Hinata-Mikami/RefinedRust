...
impl LinkedList {
    ...
    fn alloc(&mut self, value: i32) -> *mut Node {
        let node = box_new(Node {      // Boxでヒープにメモリを確保
            value,
            next: ptr::null_mut(),
            marked: false,
        });
        let ptr = box_into_raw(node);  // Boxから生ポインタに変換し所有権を放棄
    ...
    unsafe fn sweep(&mut self) {
        ...
        // 全ノードをチェックし、マークされているものだけを残す
        while self.all_nodes.len() > 0 {
            if let Some(node_ptr) = self.all_nodes.pop() {
                if (*node_ptr).marked {
                    (*node_ptr).marked = false;     // マークリセット
                    survivors.push(node_ptr);       // 生存者リストに移動
                } else {
                    println!("GC msg : Collecting Node [{}]. ", (*node_ptr).value);
                    let _ = box_from_raw(node_ptr); // 解放
                }
            }
        }
        self.all_nodes = survivors;

    }

    /* マークアンドスイープGC */
    unsafe fn collect(&mut self, mut roots: Vec<*mut Node>) {
        println!("------------------------\nGC msg : Collection started.");
    
        // 修正：forループは使えないので、pop() で一つずつ取り出して処理する
        while roots.len() > 0 {
...
fn main() {
    ...
    unsafe {
        ...
        // 修正：&[n1] ではなく、minivec::Vec を作って渡す
        let mut roots1 = Vec::new();
        roots1.push(n1);
        linkedlist.collect(roots1);       // GC : n4解放
        
        linkedlist.set_next(n1, n3);        // n1 -> n3 (n2孤立)
        
        let mut roots2 = Vec::new();
        roots2.push(n1);
        linkedlist.collect(roots2);       // GC : n2解放
