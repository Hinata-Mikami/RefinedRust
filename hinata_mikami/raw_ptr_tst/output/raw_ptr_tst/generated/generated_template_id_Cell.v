...
Definition id_Cell_lemma (π : thread_id) : Prop :=
  ⊢ typed_function π (id_Cell_def ) (<tag_type> spec! ( *[]) : 0 | ( *[]) : ([] : list RT), fn_spec_add_late_pre (type_of_id_Cell  <INST!>) (λ π, (True)
  ∗ (⌜(trait_incl_of_id_Cell <INST!>)%Z⌝))).
End proof.

Ltac id_Cell_prelude :=
  unfold id_Cell_lemma;
  set (FN_NAME := FUNCTION_NAME "id_Cell");
  intros;
  iStartProof;
  let ϝ := fresh "ϝ" in
  let p := fresh "p" in
  let v := fresh "v" in
  start_function "id_Cell" ϝ ( [] ) ( [] ) ( [  p v ] ) (  p v );
  intros arg_p;
  let π := get_π in
  let Σ := get_Σ in
