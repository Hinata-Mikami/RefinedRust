...
Section rules.
  ...
  Lemma type_mut_bor E L f (T : typed_val_expr_cont_t) e κ_name (ty_annot : option rust_type) :
    (∃ M, named_lfts M ∗ li_tactic (compute_map_lookup_nofail_goal M κ_name) (λ κ,
      (named_lfts M -∗ typed_borrow_mut E L f e κ ty_annot (λ L' v γ rt ty r,
        T L' v MetaNone (place_rfn rt * gname)%type (mut_ref κ ty) (#r, γ)))))
    ⊢ typed_val_expr E L f (&ref{Mut, ty_annot, κ_name} e)%E T.
  Proof.
    rewrite /compute_map_lookup_nofail_goal.
    iIntros "HT".
    iDestruct "HT" as "(%M & Hnamed & %κ & _ & HT)". iIntros (Φ) "#(LFT & LLCTX) #HE HL Hf HΦ".
    ...
    wpe_bind.
    iApply wpe_fupd.
    iApply (wpe_logical_step with "[HT Hat]"); [solve_ndisj.. | | ].
    ...
    iMod ("HT") as "(%L' & %rt' & %ty & %r & %γ & %ly & Hobs & Hbor & %Hst & %Hly & #Hlb & #Hsc & HL & Hf & HT)".
    iModIntro.
    ...
    iApply ("HΦ" with "HL Hf [Hcred2 Hcred1 Hat Hbor Hobs] HT").
