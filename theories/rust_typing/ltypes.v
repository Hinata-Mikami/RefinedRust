...
(* TODO move *)
Import EqNotations.
Lemma rew_UIP_RT {X : RT} (F : RT → Type) (Heq : X = X) (z : F X) :
  rew [F] Heq in z = z.
...
(**
  ...
  [Owned] says that we fully own it.
...
Global Instance bor_kind_inhabited : Inhabited bor_kind := populate (Owned).
...
Section ltype_def.
  ...
  Inductive lty : Type :=
    ...
    | AliasLty (rt : RT) (st : syn_type) (l : loc)
    | MutLty (lt : lty) (κ : lft)
    | ShrLty (lt : lty) (κ : lft)
    ...
    (* [en] is the enum spec. [variant] is the current unfolded variant. [lte] is the current type of the variant. [re] is the current refinement of the type of the variant. *)
    | EnumLty {rt rte : RT} (en : enum rt) (variant : string) (lte : lty) (re : rte)
    | OpenedLty
    ...
    | CoreableLty
        ...
        (κs : list lft)
        (lt_full : lty)
    | ShadowedLty {rt : RT} (lt_cur : lty) (r_cur : place_rfn rt) (lt_full : lty)
    | OpenedNaLty
        {rt_inner : RT}
  ...
  Fixpoint lty_rt (lt : lty) {struct lt} : RT :=
    ...
    | @OfTyLty rt _ => rt
    | AliasLty rt st l => rt
    | MutLty lt _ => (place_rfnRT (lty_rt lt) * gname)%type
  ...
  Fixpoint lty_wf (lt : lty) : Prop :=
    ...
    | @EnumLty rt rte en variant lte re =>
        lty_wf lte
        ∧ lty_rt lte = rte
  ...
  Lemma lty_recursor :
    ∀ P : lty → Type,
      (∀ (rt : RT) (t : type rt), ∀ κ : lft, P (BlockedLty t κ)) →
      (∀ (rt : RT) (t : type rt), ∀ κ : lft, P (ShrBlockedLty t κ)) →
      (∀ (rt : RT) (t : type rt), P (OfTyLty t)) →
      (∀ (rt : RT) (st : syn_type) (l : loc), P (AliasLty rt st l)) →
      ...
      (∀ (rt : RT) (def : type rt) (len : nat) (lts : list (nat * lty)),
        list_is_list _ (λ '(_, lt), P lt) lts → P (ArrayLty def len lts)) →
      (∀ (rt rte : RT) (en : enum rt) (variant : string) (lte : lty) (re : rte),
        P lte → P (EnumLty en variant lte re)) →
      (∀ (rt_inner rt_full : RT) (lt_cur lt_inner lt_full : lty)
      ...
      (∀ (rt_cur : RT) (lt_cur : lty) (r_cur : place_rfn rt_cur) (lt_full : lty), P lt_cur → P lt_full → P (ShadowedLty lt_cur r_cur lt_full)) →
      (∀ (rt_inner : RT) (lt_cur lt_inner : lty)
        (Cpre : rt_inner → iProp Σ) (Cpost : rt_inner → iProp Σ),
          P lt_cur → P lt_inner → P (OpenedNaLty lt_cur lt_inner Cpre Cpost)) →
      ∀ lt : lty, P lt.
  Proof.
    intros P Hblocked Hshrblocked Hofty Halias Hmut Hshr Hstruct Harr Henum Hopened Hcoreable Hshadow Hopened_na.
  ...
  Lemma lty_induction :
    ∀ P : lty → Prop,
      ...
      (∀ (rt : RT) (def : type rt) (len : nat) (lts : list (nat * lty)), (∀ i lt, (i, lt) ∈ lts → P lt) → P (ArrayLty def len lts)) →
      (∀ (rt rte : RT) (en : enum rt) (variant : var_name) (lte : lty) (re : rte), P lte → P (EnumLty en variant lte re)) →
      (∀ (rt_inner rt_full : RT) (lt_cur lt_inner lt_full : lty)
        (Cpre : rt_inner → rt_full → iProp Σ) (Cpost : rt_inner → rt_full → iProp Σ),
          P lt_cur → P lt_inner → P lt_full → P (OpenedLty lt_cur lt_inner lt_full Cpre Cpost)) →
      (∀ κs (lt_full : lty), P lt_full → P (CoreableLty κs lt_full)) →
  ...
  Lemma lty_induction_wf :
    ∀ P : lty → Prop,
      ...
      (∀ (rt rte : RT) (en : enum rt) (variant : var_name) (lte : lty) (re : rte),
        (P lte ∧ lty_rt lte = rte) → P (EnumLty en variant lte re)) →
  ...
  Proof.
    ...
    induction lt as [ | | | | | | lts IH sls | rt def len lts IH | rt rte en variant lte re IH | rt_inner rt_full lt_cur lt_inner lt_full Cpre Cpost IH1 IH2 IH3 | κ lt_full IH | | ] using lty_induction; [by eauto.. | | | | | | | ].
  ...
  Fixpoint lty_size (lt : lty) : nat :=
    ...
    | EnumLty en variant lte re => 1 + lty_size lte
    | OpenedLty lt_cur lt_inner lt_full Cpre Cpost =>
  ...
  Fixpoint lty_core (lt : lty) : lty :=
    ...
    | EnumLty en variant lte re => EnumLty en variant (lty_core lte) re
    | OpenedLty lt_cur lt_inner lt_full Cpre Cpost =>
  ...
  Lemma RT_eq_iff (a b : RT) :
    a = b ↔
      (∃ (Heq1 : (RT_rt a) = (RT_rt b)) (Heq2 : (RT_xt a) = (RT_xt b)),
      ∀ r : RT_xt a, (RT_xrt a r) = rew <-[id] Heq1 in (RT_xrt b (rew [id] Heq2 in r))).
  Proof.
    destruct a as [?? xrta], b as [?? xrtb]. simpl in *.
    split.
    - intros Ha. injection Ha.
      intros Hx1 <- <-.
      apply existT_inj in Hx1. apply existT_inj in Hx1 as <-.
      exists eq_refl, eq_refl.
      intros r. simpl. done.
    - intros (Heq1 & Heq2 & Heqf).
      subst. enough (xrta = xrtb) as <- by done.
  ...
  Proof.
    induction lt as [ | |  | ? IH ? | ? IH | ? IH | ? IH ? | ???? IH | | | | | ] using lty_induction; simpl; [done | done | done | done | | | | done.. ].
    - by rewrite IH.
    - by rewrite IH.
    - f_equiv.
      unfold fmap.
  ...
  Proof.
    induction lt as [ | | | | | | lts IH sls | rt def len lts IH | rt rte en variant lte re IH | | rt lt IH | | ] using lty_induction; simpl; [done.. | | | | done | | | done ].
  ...
  Definition lty_of_ty_own {rt} (ty : type rt) k π (r : place_rfn rt) l :=
    ...
    | Owned =>
        ...
        |={lftE}=>  ∃ v, l ↦ v ∗ ty.(ty_own_val) π r' MetaNone v
    | Uniq κ γ =>
        have_creds ∗
  ...
  Equations lty_own_pre (core : bool) (ltp : lty) (k : bor_kind) (π : thread_id) (r : place_rfn (lty_rt ltp)) (l : loc) : iProp Σ by wf ltp lty_size_rel :=
    ...
    lty_own_pre core (MutLty lt κ) k π r l :=
      ...
      | Owned =>
          ...
          |={lftE}=> ∃ l' : loc , l ↦ l' ∗ (lty_own_pre core lt (Uniq κ γ) π r' l')
      | Uniq κ' γ' =>
            have_creds ∗
            place_rfn_interp_mut r γ' ∗
            |={lftE}=> &pin{κ'}
      ...
      | Owned =>
          ...
          big_sepL_P (pad_struct sl.(sl_members) (pzipl lts r') (λ ly, existT (UninitLty (UntypedSynType ly)) (PlaceIn ())))
            (λ i ty HP,
              ...
              lty_own_pre core (projT1 ty) Owned π (projT2 ty) (l +ₗ Z.of_nat (offset_of_idx sl.(sl_members) i)))
      | Uniq κ γ =>
        have_creds ∗
        place_rfn_interp_mut r γ ∗
        ...
        |={lftE}=> &pin{κ}
          [∃ (r' : plist (λ lt, place_rfn (lty_rt lt)) lts),
            ...
            big_sepL_P (pad_struct sl.(sl_members) (pzipl lts r') (λ ly, existT (UninitLty (UntypedSynType ly)) (PlaceIn ())))
              (λ i ty HP,
                  ∃ ly, ⌜snd <$> sl.(sl_members) !! i = Some ly⌝ ∗ ⌜syn_type_has_layout (lty_st (projT1 ty)) ly⌝ ∗
                  lty_own_pre true (projT1 ty) Owned π (projT2 ty) (l +ₗ Z.of_nat (offset_of_idx sl.(sl_members) i)))
          ]
          (∃ (r' : plist (λ lt, place_rfn (lty_rt lt)) lts),
            gvar_auth γ r' ∗ |={lftE}=>
            big_sepL_P (pad_struct sl.(sl_members) (pzipl lts r') (λ ly, existT (UninitLty (UntypedSynType ly)) (PlaceIn ())))
        ...
        big_sepL_P (zip (interpret_iml (OfTyLty def) len lts) r')
          (λ i ty HP,
            ∃ (Heq : RT_rt (lty_rt ty.1) = RT_rt rt),
              ...
              lty_own_pre core ty.1 Owned π (rew <- [place_rfn] Heq in ty.2) (offset_loc l ly i)))
      | Uniq κ γ =>
        have_creds ∗
        place_rfn_interp_mut r γ ∗
        |={lftE}=> &pin{κ}
          [∃ r' : list (place_rfn rt), gvar_auth γ r' ∗ (|={lftE}=> ⌜length r' = len⌝ ∗
              ...
              (λ i ty HP,
                ∃ (Heq : RT_rt (lty_rt ty.1) = RT_rt rt),
                  ⌜lty_st ty.1 = ty_syn_type def MetaNone⌝ ∗
                  lty_own_pre true ty.1 Owned π (rew <- [place_rfn] Heq in ty.2) (offset_loc l ly i)
              ))]
          (∃ r' : list (place_rfn rt), gvar_auth γ r' ∗ (|={lftE}=> ⌜length r' = len⌝ ∗
              big_sepL_P (zip (interpret_iml (OfTyLty def) len lts) r')
              (λ i ty HP,
    ...
    (** EnumLty *)
      ...
      | Owned =>
        ...
        |={lftE}=>(
        (*∃ rty : enum_tag_sem rt,*)
        (*⌜lty_rt lte = rty.(enum_tag_sem_rt)⌝ ∗*)
        ⌜enum_tag en r' = Some variant⌝ ∗
        (* require all the tag info to be coherent -- in particular the refinement type *)
        (*⌜enum_tag_ty_inj en variant = Some rty⌝ ∗*)
        (*∃ (Heq : lty_rt lte = enum_variant_rt en r'),*)
        (* ownership of the discriminant *)
        lty_of_ty_own (int en.(enum_els).(els_tag_it)) Owned π (PlaceIn ((els_lookup_tag en.(enum_els) variant))) (l atst{sls_of_els en.(enum_els)}ₗ "discriminant") ∗
        (* ownership of the data *)
        (∃ Heq : RT_rt (lty_rt lte) = RT_rt rte,
          lty_own_pre core lte Owned π
        ...
        ([∗ list] i ↦ mly ∈ pad_struct el.(sl_members) [None; None] (λ ly, Some ly),
          from_option (λ ly, lty_of_ty_own (uninit (UntypedSynType ly)) Owned π (PlaceIn ()) (l +ₗ Z.of_nat (offset_of_idx el.(sl_members) i))) True mly)
        )
      | _ =>
          (* TODO *)
          False
    ...
    lty_own_pre core (@OpenedLty rt_inner rt_full lt_cur lt_inner lt_full Cpre Cpost) k π r l :=
      ...
      match k with
      | Owned =>
          lty_own_pre false lt_cur Owned π r l ∗
          logical_step lftE
          (∃ (Heq_inner : rt_inner = lty_rt lt_inner) (Heq_full : rt_full = lty_rt lt_full),
            ...
            ∀ (r : rt_inner) (r' : rt_full) (κs : list lft),
              ...
              Cpost r r' ∗
              (lft_dead_list κs -∗
               lty_own_pre false lt_inner Owned π (PlaceIn (rew [RT_rt] Heq_inner in r)) l ={lftE}=∗
               lty_own_pre true lt_full Owned π (PlaceIn (rew [RT_rt] Heq_full in r')) l
              ))
      | Uniq κ γ =>
        lty_own_pre false lt_cur Owned π r l ∗
        ...
        (∃ (Heq : rt_inner = lty_rt lt_inner) (Heq_full : rt_full = lty_rt lt_full),
          ...
          ∀ (own_lt_cur' : thread_id → rt_inner → loc → iProp Σ) (κs : list lft)
            ...
            (□ (lft_dead_list κs -∗ own_lt_cur' π r l ={lftE}=∗
              lty_own_pre true lt_inner Owned π (rew [place_rfnRT] Heq in PlaceIn r) l)) ={lftE}=∗

            Cpost r r' ∗
            gvar_obs γ (rew [RT_rt] Heq_full in r') ∗
            (lft_dead_list κs -∗ gvar_obs γ (rew [RT_rt] Heq_full in r') ={lftE}=∗
              lty_own_pre true lt_full (Uniq κ γ) π (rew [place_rfnRT] Heq_full in PlaceIn r') l)
        )
    ...
    lty_own_pre core (@CoreableLty κs lt_full) k π r l :=
      (if core then
        ...
        match k with
        | Owned =>
            lft_dead_list κs ={lftE}=∗ lty_own_pre true lt_full Owned π r l
        | Uniq κ' γ =>
            place_rfn_interp_mut r γ ∗
    ...
    lty_own_pre core (@OpenedNaLty rt_inner lt_cur lt_inner Cpre Cpost) k π r l :=
      ...
      | Owned =>
          ...
          (∃ (Heq_inner : rt_inner = lty_rt lt_inner),
            ∀ (r : rt_inner),
              Cpre r -∗
              lty_own_pre false lt_inner Owned π (PlaceIn (rew [RT_rt] Heq_inner in r)) l ={lftE ∪ shrE}=∗ Cpost r)
      | _ => False
      end
  .
  ...
  Proof.
    ...
    induction lt as [ | | | | lt IH κ | lt IH κ | lts IH sls | rt def len lts IH | rt rte en variant lt re IH | | | | ] using lty_induction in k, π, r, l, Heq |-*; simpl in *.
  ...
  Proof.
    ...
    induction lt as [ | | | | lt IH κ | lt IH κ | lts IH sls | def len lts IH IH' | rt rte en variant lt re IH | | | | ] using lty_induction in k, π, r, l, Heq, core |-*; simpl in *.
  ...
  Proof.
    rewrite /lty_own_core /lty_own.
    induction lt as [ | | | | lt IH κ | lt IH κ | lts IH sls | rt def len lts IH  | rt en variant lt IH | | | ???? IH1 IH2 | ] using lty_induction in κ0, π, r, l, Heq |-*; simpl in *.
  ...
  Program Definition AliasLtype (rt : RT) (st : syn_type) (l : loc) : ltype rt := {|
  ...
  Program Definition MutLtype {rt} (lt : ltype rt) (κ : lft) : ltype (prodRT (place_rfnRT rt) gnameRT)%type := {|
  ...
  Program Definition ShrLtype {rt} (lt : ltype rt) (κ : lft) : ltype (place_rfnRT rt) := {|
  ...
  Program Definition ArrayLtype {rt : RT} (def : type rt) (len : nat) (lts : list (nat * ltype rt)) : ltype (listRT (place_rfnRT rt)) := {|
  ...
  Program Definition EnumLtype {rt rte : RT} (en : enum rt) (variant : string)
      (lte : ltype rte) (re : rte) : ltype rt := {|
    ltype_lty := @EnumLty rt rte en variant lte.(ltype_lty) re;
  |}.
  Next Obligation.
    intros rt rte en variant lte re. simpl. done.
  Qed.
  Next Obligation.
    intros rt rte en variant lte re. simpl. split_and!.
    - apply ltype_lty_wf.
  ...
  Program Definition OpenedLtype {rt_cur rt_inner rt_full : RT} (lt_cur : ltype rt_cur) (lt_inner : ltype rt_inner) (lt_full : ltype rt_full)
  ...
  Definition ltype_own_pre (core : bool) {rt} (lt : ltype rt) : bor_kind → thread_id → place_rfn rt → loc → iProp Σ :=
    λ k π r l, lty_own_pre core lt.(ltype_lty) k π (rew <- [place_rfnRT] lt.(ltype_rt_agree) in r) l.
  ...
  Section induction.
    ...
    Lemma ltype_induction (P : ∀ rt, ltype rt → Prop) :
      (∀ (rt : RT) (t : type rt) κ, P _ (BlockedLtype t κ)) →
      (∀ (rt : RT) (t : type rt) κ, P _ (ShrBlockedLtype t κ)) →
      (∀ (rt : RT) (t : type rt), P _ (OfTy t)) →
      (∀ (rt : RT) (st : syn_type) (l : loc), P _ (AliasLtype rt st l)) →
      (∀ (rt : RT) (lt : ltype rt), P _ lt → ∀ κ, P _ (MutLtype lt κ)) →
      (∀ (rt : RT) (lt : ltype rt), P _ lt → ∀ κ, P _ (ShrLtype lt κ)) →
      (∀ (rts : list RT) (lts : hlist ltype rts),
      ...
      (∀ (rt rte : RT) (en : enum rt) (variant : var_name)
        (lte : ltype rte) (re : rte),
        P _ lte → P _ (EnumLtype en variant lte re)) →
      (∀ (rt_cur rt_inner rt_full : RT) (lt_cur : ltype rt_cur) (lt_inner : ltype rt_inner) (lt_full : ltype rt_full)
        (Cpre : rt_inner → rt_full → iProp Σ) (Cpost : rt_inner → rt_full → iProp Σ),
        P _ lt_cur → P _ lt_inner → P _ lt_full →
        P _ (OpenedLtype lt_cur lt_inner lt_full Cpre Cpost)) →
      (∀ (rt_full : RT) κs (lt_full : ltype rt_full), P _ lt_full → P _ (CoreableLtype κs lt_full)) →
      (∀ (rt_cur rt_full : RT) (lt_cur : ltype rt_cur) (r_cur : place_rfn rt_cur) (lt_full : ltype rt_full),
        P _ lt_cur → P _ lt_full → P _ (ShadowedLtype lt_cur r_cur lt_full)) →
      (∀ (rt_cur rt_inner : RT) (lt_cur : ltype rt_cur) (lt_inner : ltype rt_inner)
        (Cpre : rt_inner → iProp Σ) (Cpost : rt_inner → iProp Σ),
        P _ lt_cur → P _ lt_inner →
        P _ (OpenedNaLtype lt_cur lt_inner Cpre Cpost)) →
      ∀ (rt : RT) (lt : ltype rt), P _ lt.
    Proof.
      intros Hblocked Hshrblocked Hofty Halias Hmut Hshr Hstruct Harr Hen Hopened Hcoreable Hshadow Hopened_na.
      ...
      intros rt [lt <- Hwf].
      induction lt as [ | | | | lt IH κ | lt IH κ | lts IH sls | rt def len lts IH | rt rte en variant lte re IH | rt_inner rt_full lt_cur lt_inner lt_full Cpre Cpost IH_cur IH_inner IH_full | κ lt_full IH | rt_cur lt_cur r_cur lt_full IH_cur IH_full | rt_inner lt_cur lt_inner Cpre Cpost IH_cur IH_inner ] using lty_induction; simpl.
      ...
      - (* enum *)
        ...
        specialize (Hen rt rte en variant ltypee re).
        eapply P_irrel.
        apply Hen.
        subst ltypee.
        move: IH.
        clear.
        move: Hrt.
        (*generalize (enum_tag_rt' en variant). *)
        intros ??. subst.
        done.
  ...
  Definition mut_ltype_own
    (rec : ltype_own_type)
    (rec_core : ltype_own_type)
    {rt} (lt : ltype rt) (κ : lft) (k : bor_kind) (π : thread_id) (r : place_rfn (place_rfn rt * gname)%type) (l : loc) : iProp Σ :=
    (∃ ly : layout, ⌜syn_type_has_layout PtrSynType ly⌝ ∗ ⌜l `has_layout_loc` ly⌝ ∗
       loc_in_bounds l 0 ly.(ly_size) ∗
      match k with
      | Owned =>
          ...
          place_rfn_interp_owned r (r', γ) ∗
          (* TODO layout requirements on l' here? *)
          |={lftE}=>  ∃ l' : loc , l ↦ l' ∗ (rec _ lt (Uniq κ γ) π r' l')
      | Uniq κ' γ' =>
          have_creds ∗
          place_rfn_interp_mut r γ' ∗
          |={lftE}=> &pin{κ'}
  ...
  Definition struct_make_uninit_ltype (ly : layout) : sigT (λ rt : RT, (ltype rt * place_rfn rt)%type) :=
    existT (unit : RT) (UninitLtype (UntypedSynType ly), PlaceIn ()).
  ...
  Definition struct_ltype_own
    ...
    (k : bor_kind) (π : thread_id) (r : place_rfn (plist place_rfnRT rts)) (l : loc) : iProp Σ :=
    ∃ sl : struct_layout,
    ⌜use_struct_layout_alg sls = Some sl⌝ ∗
    ...
    match k with
    | Owned =>
        ∃ r' : plist place_rfnRT rts, place_rfn_interp_owned r r' ∗ |={lftE}=>
          [∗ list] i ↦ ty ∈ pad_struct sl.(sl_members) (hpzipl rts lts r') struct_make_uninit_ltype,
            ...
            rec _ (projT2 ty).1 Owned π (projT2 ty).2 (l +ₗ Z.of_nat (offset_of_idx sl.(sl_members) i))
    | Uniq κ γ =>
        have_creds ∗
        place_rfn_interp_mut r γ ∗
        (* We change the ownership to Owned false, because we can't push the borrow down in this formulation of products.
          ...
          which propagates to all the other Uniq cases of other ltypes. *)
        |={lftE}=> &pin{κ}
          [∃ (r' : plist place_rfnRT rts),
            gvar_auth γ r' ∗ |={lftE}=>
            [∗ list] i ↦ ty ∈ pad_struct sl.(sl_members) (hpzipl rts lts r') struct_make_uninit_ltype,
              ∃ ly, ⌜snd <$> sl.(sl_members) !! i = Some ly⌝ ∗ ⌜syn_type_has_layout (ltype_st (projT2 ty).1) ly⌝ ∗
              rec_core _ (projT2 ty).1 Owned π (projT2 ty).2 (l +ₗ Z.of_nat (offset_of_idx sl.(sl_members) i))
          ]
          (∃ (r' : plist place_rfnRT rts),
            gvar_auth γ r' ∗ |={lftE}=>
            [∗ list] i ↦ ty ∈ pad_struct sl.(sl_members) (hpzipl rts lts r') struct_make_uninit_ltype,
    ...
    ∃ ly,
      ...
      | Owned =>
          ...
          [∗ list] i ↦ lt; r0 ∈ (interpret_iml (OfTy def) len lts); r',
              ⌜ltype_st lt = ty_syn_type def MetaNone⌝ ∗ rec _ lt Owned π r0 (offset_loc l ly i))
      | Uniq κ γ =>
        have_creds ∗
        place_rfn_interp_mut r γ ∗
        |={lftE}=> &pin{κ}
          [∃ r' : list (place_rfn rt), gvar_auth γ r' ∗ (|={lftE}=> ⌜length r' = len⌝ ∗
              [∗ list] i ↦ lt; r0 ∈ interpret_iml (OfTy def) len lts; r',
                ⌜ltype_st lt = ty_syn_type def MetaNone⌝ ∗ rec_core _ lt Owned π r0 (offset_loc l ly i))]
          (∃ r' : list (place_rfn rt), gvar_auth γ r' ∗ (|={lftE}=> ⌜length r' = len⌝ ∗
              [∗ list] i ↦ lt; r0 ∈ interpret_iml (OfTy def) len lts; r',
                ⌜ltype_st lt = ty_syn_type def MetaNone⌝ ∗ rec _ lt Owned π r0 (offset_loc l ly i)))
      | Shared κ =>
  ...
  Definition enum_ltype_own
    (rec : ltype_own_type)
    (rec_core : ltype_own_type)
    (rt rte : RT) (en : enum rt)
    (tag : var_name) (lte : ltype rte) (re : rte)
    (k : bor_kind) (π : thread_id) (r : place_rfn rt) (l : loc) : iProp Σ :=
      ...
      | Owned =>
        ...
        (*⌜is_Some (enum_tag_ty_inj en tag)⌝ ∗*)
        (* ownership of the discriminant *)
        lty_of_ty_own (int en.(enum_els).(els_tag_it)) Owned π (PlaceIn (els_lookup_tag en.(enum_els) tag)) (l atst{sls_of_els en.(enum_els)}ₗ "discriminant") ∗
        (* ownership of the data *)
        rec _ lte Owned π
        (PlaceIn re)
        (*(rew [place_rfn] Heq in (PlaceIn (enum_variant_rfn en r')))*)
        ...
        (* ownership of the padding fields of the sl *)
        ([∗ list] i ↦ mly ∈ pad_struct el.(sl_members) [None; None] (λ ly, Some ly),
        if mly is Some ly then lty_of_ty_own (uninit (UntypedSynType ly)) Owned π (PlaceIn ()) (l +ₗ Z.of_nat (offset_of_idx el.(sl_members) i)) else True))
      | _ =>
          (* TODO *)
          False
      end.
    ...
    ∃ ly, ⌜use_layout_alg (ltype_st lt_cur) = Some ly⌝ ∗
      ...
      | Owned =>
          ...
          (∀ (r : rt_inner) (r' : rt_full) (κs : list lft),
            ...
            Cpost r r' ∗
            (lft_dead_list κs -∗
             ltype_own lt_inner Owned π (PlaceIn r) l ={lftE}=∗
             ltype_own_core lt_full Owned π (PlaceIn r') l))
      | Uniq κ γ =>
        ltype_own lt_cur Owned π r l ∗
          (* we will execute this VS when closing the invariant, after having already stratified [lt_cur].
          ...
          (∀ (own_lt_cur' : thread_id → rt_inner → loc → iProp Σ) (κs : list lft) (r : rt_inner) (r' : rt_full),
            ...
            (* the ownership of [lt_cur'] needs to be shiftable to the _core_ of [lt_inner];
              this is required for closing the borrow and for proving that it can be unblocked to lt_full
                (which is needed for showing the shift to Coreable) *)
            (□ (lft_dead_list κs -∗ own_lt_cur' π r l ={lftE}=∗
              ltype_own_core lt_inner Owned π (PlaceIn r) l)) ={lftE}=∗
  ...
  Definition coreable_ltype_own (rec : ltype_own_type) (rec_core : ltype_own_type)
    ...
    match k with
    | Owned =>
        lft_dead_list κs ={lftE}=∗ rec_core _ lt_full Owned π r l
    | Uniq κ' γ =>
        place_rfn_interp_mut r γ ∗
  ...
  Definition opened_na_ltype_own
    ...
    ∃ ly, ⌜use_layout_alg (ltype_st lt_cur) = Some ly⌝ ∗
      ...
      | Owned =>
            ...
            (∀ (r : rt_inner),
              Cpre r -∗
              ltype_own lt_inner Owned π (PlaceIn r) l ={lftE ∪ shrE}=∗ Cpost r)
      | _ => False
    end.
  ...
  Lemma ltype_own_pre_alias_unfold (rt : RT) (st : syn_type) (p : loc) (core : bool) k π r l :
  ...
  Qed.
  Lemma ltype_own_alias_unfold (rt : RT) (st : syn_type) (p : loc) k π r l :
  ...
  Lemma ltype_own_core_alias_unfold (rt : RT) (st : syn_type) (p : loc) k π r l :
  ...
  Lemma StructLtype_StructLty_rfn_eq (rts : list RT) (lts : hlist ltype rts) :
    plist place_rfnRT rts = plist (λ lt : lty, place_rfn (lty_rt lt)) (@ltype_lty +c<$> lts).
  Proof.
  ...
  Section structlty.
    ...
    Local Definition map_fun : sigT (λ rt : RT, (ltype rt * place_rfn rt)%type) → sigT (λ lt : lty, place_rfn (lty_rt lt)) :=
      (λ x, existT (ltype_lty (projT2 x).1) (rew <- [place_rfnRT] ltype_rt_agree (projT2 x).1 in (projT2 x).2)).


    Local Lemma StructLtype_fmap_eq {rts : list RT} (lts : hlist ltype rts) (r : plist place_rfnRT rts) (Heq: plist (λ lt : lty, place_rfn (lty_rt lt)) (@ltype_lty +c<$> lts) = plist place_rfnRT rts):
    ...
    Local Lemma StructLtype_big_sepL_fmap' {rts : list RT} (sl : struct_layout) (lts : hlist ltype rts) (r : plist place_rfnRT rts) r'
      (Heq2: plist (λ lt : lty, place_rfn (lty_rt lt)) (@ltype_lty +c<$> lts) = plist place_rfnRT rts)
      ...
      ([∗ list] i ↦ ty ∈ pad_struct sl.(sl_members) (hpzipl rts lts r)
          (λ ly : layout, existT (unit : RT) (UninitLtype (UntypedSynType ly), PlaceIn tt)),
        ∃ ly, ⌜snd <$> sl.(sl_members) !! i = Some ly⌝ ∗ ⌜syn_type_has_layout (ltype_st (projT2 ty).1) ly⌝ ∗
       lty_own_pre core (ltype_lty (projT2 ty).1) k π (rew <-[place_rfnRT] ltype_rt_agree (projT2 ty).1 in (projT2 ty).2) (l +ₗ offset_of_idx sl.(sl_members) i)).
    Proof.
      intros ->.
      rewrite StructLtype_fmap_eq.
      rewrite (pad_struct_ext _ _ _ (λ ly : layout, map_fun (existT (unit : RT) (UninitLtype (UntypedSynType ly), PlaceIn ())))).
      2: { intros. unfold map_fun. simpl. f_equiv.
        match goal with
        |- PlaceIn _ = rew <- [_] ?Heq in _ => generalize Heq
        end.
        simpl. intros Heq3.
  ...
  Proof.
    rewrite ?ltype_own_unseal.
    rewrite /ltype_own_def ltype_own_pre_struct_unfold. done.
  Qed.
  Lemma ltype_own_core_struct_unfold {rts : list RT} (lts : hlist ltype rts) (sls : struct_layout_spec) k π r l :
    ltype_own_core (StructLtype lts sls) k π r l ≡ struct_ltype_own (@ltype_own_core) (@ltype_own_core) lts sls k π r l.
  Proof.
    rewrite {1 2} ltype_own_core_unseal.
    rewrite /ltype_own_core_def ltype_own_pre_struct_unfold. done.
  Qed.
  ...
  Proof. done. Qed.
  Lemma ltype_own_pre_array_unfold {rt : RT} (def : type rt) (len : nat) (lts : list (nat * ltype rt)) (core : bool) k π r l :
  ...
  Lemma ltype_own_array_unfold {rt : RT} (def : type rt) (len : nat) (lts : list (nat * ltype rt)) k π r l :
  ...
  Lemma ltype_own_core_array_unfold {rt : RT} (def : type rt) (len : nat) (lts : list (nat * ltype rt)) k π r l :
  ...
  Lemma ltype_own_pre_enum_unfold {rt rte : RT} (en : enum rt) (tag : string) (lte : ltype rte) (re : rte) (core : bool) k π r l :
    ltype_own_pre core (EnumLtype en tag lte re) k π r l ≡ enum_ltype_own (@ltype_own_pre core) (@ltype_own_core) rt rte en tag lte re k π r l.
  Proof.
    rewrite /enum_ltype_own ?ltype_own_core_unseal /ltype_own_core_def ?ltype_own_unseal /ltype_own_def /ltype_own_pre.
    simp lty_own_pre.
    generalize (ltype_rt_agree (EnumLtype en tag lte re)).
    fold lty_rt. simpl.
    intros Heq1. rewrite (UIP_refl _ _ Heq1). cbn.
    do 7 f_equiv.
    intros r'.
    ...
    simpl.
    iSplit.
    - iIntros "(%Heq & ?)". rewrite (UIP_refl _ _ Heq). done.
    - iIntros "?". iExists eq_refl. done.
      (*
    iSplit.
    - iIntros "(%rty & %Hrty & %Htag & %Htag2 & Hl)".
      destruct rty as [rte tye ?]. simpl.
    ...
    - iIntros "(%Hels & %Htag & %Htag2 & Hdisc & Hdata & Hl)".
      destruct Htag2 as [[rte tye inje] Htag2].
      iExists (mk_enum_tag_sem rte tye inje).
      iFrame "%".
      odestruct enum_tag_compat as (einj' & Htag'); first apply Htag.
      rewrite Htag' in Htag2.
  ...
  Lemma ltype_own_enum_unfold {rt rte : RT} (en : enum rt) (tag : string) (lte : ltype rte) (re : rte) k π r l :
    ltype_own (EnumLtype en tag lte re) k π r l ≡ enum_ltype_own (@ltype_own) (@ltype_own_core) rt rte en tag lte re k π r l.
  Proof. rewrite ?ltype_own_unseal. apply ltype_own_pre_enum_unfold. Qed.
  Lemma ltype_own_core_enum_unfold {rt rte : RT} (en : enum rt) (tag : string) (lte : ltype rte) (re : rte) k π r l :
    ltype_own_core (EnumLtype en tag lte re) k π r l ≡ enum_ltype_own (@ltype_own_core) (@ltype_own_core) rt rte en tag lte re k π r l.
  Proof. rewrite {1 2} ltype_own_core_unseal. apply ltype_own_pre_enum_unfold. Qed.

  Lemma ltype_own_opened_unfold {rt_cur rt_inner rt_full : RT} (lt_cur : ltype rt_cur) (lt_inner : ltype rt_inner) (lt_full : ltype rt_full)
  ...
  Proof.
    ...
    - iSplit.
      ...
      + iIntros "Ha". iExists eq_refl, eq_refl. done.
  Qed.
  Lemma ltype_own_core_opened_unfold {rt_cur rt_inner rt_full : RT} (lt_cur : ltype rt_cur) (lt_inner : ltype rt_inner) (lt_full : ltype rt_full)
      (Cpre : rt_inner → rt_full → iProp Σ) (Cpost : rt_inner → rt_full → iProp Σ) k π r l :
  ...
  Lemma ltype_own_opened_na_unfold {rt_cur rt_inner : RT} (lt_cur : ltype rt_cur) (lt_inner : ltype rt_inner)
  ...
  Qed.
  Lemma ltype_own_core_opened_na_unfold {rt_cur rt_inner : RT} (lt_cur : ltype rt_cur) (lt_inner : ltype rt_inner)
      (Cpre : rt_inner → iProp Σ) (Cpost : rt_inner → iProp Σ) k π r l :
  ...
  Proof.
    ...
    - apply proof_irrelevance.
  Qed.
  Lemma ltype_core_alias (rt : RT) (st : syn_type) (p : loc) :
  ...
  Qed.
  Lemma ltype_core_enum {rt rte} (en : enum rt) (tag : string) (lte : ltype rte) (re : rte) :
    ltype_core (EnumLtype en tag lte re) = EnumLtype en tag (ltype_core lte) re.
  Proof.
  ...
  Fixpoint lty_blocked_lfts (lt : lty) : list lft :=
    ...
    | EnumLty en tag lte re => lty_blocked_lfts lte
    | OpenedLty _ _ _ _ _  => []
    | CoreableLty κs _ => κs
...
Ltac simp_ltype_core Heq :=
  ...
  | _ = ltype_core (EnumLtype _ _ _ _) =>
...
(** ** Ltype subtyping *)
Definition ltype_incl' `{!typeGS Σ} {rt1 rt2 : RT} (b : bor_kind) r1 r2 (lt1 : ltype rt1) (lt2 : ltype rt2) : iProp Σ :=
  (□
    match b with
    | Owned =>
      ∀ π l, ltype_own lt1 Owned π r1 l ={lftE}=∗ ltype_own lt2 Owned π r2 l
    | Uniq κ γ =>
...
Definition ltype_eq `{!typeGS Σ} {rt1 rt2 : RT} (b : bor_kind) r1 r2 (lt1 : ltype rt1) (lt2 : ltype rt2) : iProp Σ :=
...
Section eqltype.
  ...
  Lemma type_incl_ltype_owned_wand {rt} (ty1 ty2 : type rt) π l :
    ...
    ∀ r, l ◁ₗ[π, Owned] r @ (◁ ty1) -∗ l ◁ₗ[π, Owned] r @ (◁ ty2).
  Proof.
    iIntros "#Hincl" (r) "Hl".
  ...
  Lemma type_ltype_incl_owned_in_strong {rt1 rt2} π r1 r2 (ty1 : type rt1) (ty2 : type rt2) :
    (∀ m, ty1.(ty_syn_type) m = ty2.(ty_syn_type) m) →
    (∀ v, v ◁ᵥ{π, MetaNone} r1 @ ty1 -∗ v ◁ᵥ{π, MetaNone} r2 @ ty2) -∗
    (ty1.(ty_sidecond) -∗ ty2.(ty_sidecond)) -∗
    ∀ l, l ◁ₗ[π, Owned] #r1 @ (◁ ty1) -∗ l ◁ₗ[π, Owned] #r2 @ (◁ ty2)%I.
  Proof.
    ...
    iExists ly. rewrite Hst. iFrame.
    iSplitL "Hsceq Hsc". { by iApply "Hsceq". }
    iExists r2. iSplitR; first done. iMod "Hb" as "(% & ? & Hv)". iExists _. iFrame.
    iModIntro. iDestruct "Hrfn" as "->". by iApply "Hsub".
  Qed.

  Lemma type_ltype_incl_owned_in {rt1 rt2} r1 r2 (ty1 : type rt1) (ty2 : type rt2) :
    type_incl r1 r2 ty1 ty2 -∗ ltype_incl Owned #r1 #r2 (◁ ty1)%I (◁ ty2)%I.
  Proof.
    iIntros "Hsub".
    iDestruct ("Hsub") as "(%Hst & #Hsceq & #Hown & #Hshr)".
  ...
  Qed.
  Lemma subtype_subltype_owned_in E L {rt1 rt2} r1 r2 (ty1 : type rt1) (ty2 : type rt2) :
    subtype E L r1 r2 ty1 ty2 → subltype E L Owned #r1 #r2 (◁ ty1)%I (◁ ty2)%I.
  Proof.
    iIntros (Hsub qL) "HL #CTX #HE". iPoseProof (Hsub with "HL HE") as "#Hsub".
    by iApply (type_ltype_incl_owned_in).
  Qed.

  Lemma type_ltype_incl_owned {rt : RT} (ty1 : type rt) (ty2 : type rt) :
    (∀ r, type_incl r r ty1 ty2) -∗ ∀ r, ltype_incl Owned r r (◁ ty1)%I (◁ ty2)%I.
  Proof.
    iIntros "#Hsub" (r).
    iPoseProof ("Hsub" $! inhabitant) as "(%Hst & #Hsceq & _)".
    ...
    iExists ly. rewrite Hst. iFrame.
    iSplitL "Hsc"; first by iApply "Hsceq".
    iFrame. iMod "Hb" as "(% & ? & Hv)". iExists _. iFrame.
    iModIntro. by iApply "Hown".
  Qed.
  Lemma subtype_subltype_owned E L {rt : RT} (ty1 : type rt) (ty2 : type rt) :
    (∀ r, subtype E L r r ty1 ty2) → ∀ r, subltype E L Owned r r (◁ ty1)%I (◁ ty2)%I.
  Proof.
  ...
  Lemma full_eqltype_subltype_l E L {rt : RT} (lt1 lt2 : ltype rt) :
  ...
  Qed.
  Lemma full_eqltype_subltype_r E L {rt : RT} (lt1 lt2 : ltype rt) :
...
Section blocked.
  ...
  Section unblockable.
    ...
    Definition imp_unblockable (κs : list lft) (lt : ltype rt) : iProp Σ :=
      ...
      (** Owned. *)
      (∀ π r l,
        lft_dead_list κs -∗ ltype_own lt Owned π r l ={lftE}=∗ ltype_own_core lt Owned π r l)
  ...
  Proof.
    ...
    - iIntros (π r l) "(#Hdead & _) Hblocked".
      rewrite ltype_own_core_equiv /=. simp_ltypes.
      rewrite ltype_own_shrblocked_unfold ltype_own_ofty_unfold.
      iDestruct "Hblocked" as "(%ly & ? & ? & ? & ? & %r' & Hrfn & Hshr & Hunblock)".
    ...
    - iIntros (κ' γ' π r l) "#Hdead Hb".
      ...
      + iNext. iModIntro. iSplit.
        * iIntros "(%r' & ? & Ha)". iExists r'. iFrame.
          ...
          { simpl in *.
            apply (hpzipl_lookup_inv_hzipl_pzipl _ _ r') in Hlook1 as (Hlook1 & _).
  ...
  Lemma enum_ltype_imp_unblockable {rt rte} κs (en : enum rt) (tag : string) (lte : ltype rte) (re : rte) :
    imp_unblockable κs lte -∗
    imp_unblockable κs (EnumLtype en tag lte re).
  Proof.
    iIntros "#Hub". iModIntro. iSplitL.
    - iIntros (κ' γ' π r l) "#Hdead Hb".
      rewrite ltype_own_core_equiv /=. simp_ltypes.
    ...
    - iIntros (π r l) "#Hdead Hb".
      rewrite ltype_own_core_equiv /=. simp_ltypes.
      rewrite !ltype_own_enum_unfold /enum_ltype_own.
      iDestruct "Hb" as "(%el & %Halg & % & Hlb & %r' & Hr & Hb)".
      iExists el. iR. iR. iFrame "Hlb ".
      iModIntro. iExists _. iFrame "Hr".
      iMod "Hb" as "(%Htag' & ? & Hb & ? & ?)".
      iR. rewrite ltype_core_syn_type_eq. iFrame.
  ...
  *)
    ...
    - iIntros (rt def len lts IH ).
      ...
      + iApply imp_unblockable_shorten; first last.
        ...
        * simpl. rewrite lft_dead_list_app. iDestruct "Hdead" as "(_ & Hdead)".
          by iApply "IH".
    - iIntros (rt rte en tag lte re Hub).
  ...
  Proof.
    ...
    - iIntros (π r l) "#Hdead Hb".
      iDestruct ("Heq" $! (Owned) r) as "((% & #Hi1 & #Hc1) & (_ & #Hi2 & #Hc2))".
