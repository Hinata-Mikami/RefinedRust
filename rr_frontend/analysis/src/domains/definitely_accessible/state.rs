...
impl<'tcx> DefinitelyAccessibleState<'tcx> {
    ...
    pub fn check_invariant(&self, location: impl fmt::Debug) {
        for &owned_place in &self.definitely_owned {
            debug_assert!(
                self.definitely_accessible
                    .iter()
...
fn pretty_print_place<'tcx>(tcx: TyCtxt<'tcx>, body: &mir::Body<'tcx>, place: Place<'tcx>) -> Option<String> {
    ...
    for elem in place.projection.iter().rev() {
        match elem {
            mir::ProjectionElem::Deref => pieces.push("(*".to_owned()),
            mir::ProjectionElem::Field(..) => pieces.push("(".to_owned()),
            _ => {},
        }
    }
...
fn describe_field_from_ty(
    ...
    if ty.is_box() {
        ...
        match *ty.kind() {
            ...
            | ty::TyKind::Slice(ty) => describe_field_from_ty(tcx, ty, field, variant_index),
